testing<-ggplot(EFFpp, aes(x=blocks, y=efficiency, color=factor(bb), shape=factor(bb)))  + geom_point() + 
labs(title="1KB Partial Paging", x="Proportion of time blocked", y="Efficiency of execution") +
...
testing2<-ggplot(EFFtr, aes(x=blocks, y=efficiency, color=factor(bb), shape=factor(bb)))  + geom_point() + 
labs(title="1KB Traditional Paging", x="Proportion of time blocked", y="Efficiency of execution") +

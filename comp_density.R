...
faults<-ggplot() + geom_point(aes(y=Count, x=Blocks, shape=factor(1), col=factor(1)), data=PP512[which(PP512$bb == i),])  +
geom_point(aes(y=Count, x=Blocks, shape=factor(2), col=factor(2)), data=PP256[which(PP256$bb == i),]) +
